...
(function () {
    ...
    Topic.ScrollToBookmark = function (id) {
        id = id.replace(/([ #;?%&,.+*~\':"!^$[\]()=>|\/@])/g, '\\$1'); // escape invalid jquery characters
        var $target = $("#" + id);
        if ($target.length == 0)
            $target = $("[name = '" + id + "']");
    ...
    function ScrollToOffset(targetOffset) {
        ...
        var titleBarOffset = $(".title-bar.sticky").length > 0 ? $(".title-bar.sticky").innerHeight() : 0;
        var computedOffset = targetOffset - titleBarOffset;
        var absPos = $(".sidenav-container").is(":visible") ? $scrollContainer.scrollTop() + computedOffset : computedOffset;

        $scrollContainer.scrollTop(absPos);
    }
    ...
    Topic.Init = function (context) {
        ...
        $(".MCPopupThumbnailPopup", context).click(MadCap.Topic.ThumbPopup_Click);
        $(".MCPopupThumbnailHover", context).mouseover(MadCap.Topic.ThumbPopup_Hover);

        $("a.MCHelpControl", context).click(MadCap.Topic.HelpControl_Click);
        ...
        $(".expand-all-button", context).click(function (e) {
            var $this = $(this);
        ...
        $(".remove-highlight-button", context).click(function (e) {
            RemoveHighlight();
        });

        $(".previous-topic-button", context).click(function (e) {
            MadCap.Utilities.CrossFrame.PostMessageRequest(parent, "navigate-previous");
        });

        $(".next-topic-button", context).click(function (e) {
